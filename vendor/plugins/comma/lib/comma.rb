...
class Array
  def to_comma(style = :default)
    ...
    if style.is_a? Hash
      ...
      custom_data = options.delete(:custom_data)||:default
    end
    ...
    FasterCSV.generate(options) do |csv|
      ...
      each do |object| 
        object.custom_csv_data = custom_data
        csv << object.to_comma(style, custom_data)
      end
    end
  end
end
...
class Object
  ...
  def to_comma(style = :default, custom_data = {})
    ...
    Comma::DataExtractor.new(self, custom_data, &self.comma_formats[style]).results
  end
