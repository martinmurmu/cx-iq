class NpsReportsController < ApplicationController
...
#  before_filter :authenticate_user! 
  ...
  def mail
    report = NpsReport.find(params[:id])
  ...
  def refresh
    ...
    if params[:per_page]
      ...
      elsif params[:sort_by] == 'last_update'
    ...
    render :update do |page|
      page.replace_html 'product_report', :partial => 'nps_reports/results'
    end
  end
  
  def new
    @nps_report = NpsReport.new :user_id => current_user.id
    unless params[:selected_category].blank?
      @nps_report.product_category = ProductGroup.find params[:selected_category]
    end
    setup_new_report
    respond_to do |format|
      if @nps_report.save
        session[:nps_report_id] = @nps_report.id if current_user.guest?
        #flash[:notice] = 'ProductReport was successfully created.'
        format.html { redirect_to(nps_report_products_path @nps_report) }
        format.xml  { render :xml => @nps_report, :status => :created, :location => @nps_report }
      else
        setup_new_report
        format.html { render :action => "new" }
        format.xml  { render :xml => @nps_report.errors, :status => :unprocessable_entity }
      end
    end
  end
  ...
  def edit
    ...
    if current_user.guest?
      @nps_report = current_user.nps_report
    else
      @nps_report = current_user.nps_reports.find(params[:id])
    end
    render 'new'
  end
  ...
  def create
    ...
    @nps_report = NpsReport.new(params[:product_report].merge({:user_id => current_user.id}))
    @nps_report.category = group
    unless params[:report_manufacturer_ids].blank? || params[:report_manufacturer_ids].include?("all")
      params[:report_manufacturer_ids].each{|id|
        @nps_report.manufacturers << Manufacturer.find(id)
      }
    end
  ...
  def update
    ...
    ReportManufacturer.delete_all ["report_id = ?", @nps_report.id]
    ...
    respond_to do |format|
      if @nps_report.update_attributes(params[:product_report])
